...
## 6b. Deploy hằng ngày — chỉ cần commit + push

Sau khi đã import repo vào Vercel (mục 6), quy trình cập nhật website là **tự động**:

```bash
git add -A
git commit -m "Nội dung thay đổi"
git push            # đẩy lên nhánh main
```

Mỗi lần `push` lên **main** sẽ tự động:

1. **Vercel** build & deploy code mới (qua Git integration sẵn có của Vercel).
2. **GitHub Actions** ([`.github/workflows/run-sql.yml`](.github/workflows/run-sql.yml))
   chạy toàn bộ SQL trong `supabase/` lên Supabase — **chỉ khi** commit có đụng vào
   `supabase/*.sql` (SQL viết idempotent nên chạy lại nhiều lần vẫn an toàn).

### Cấu hình 1 lần cho GitHub Actions

Vào repo trên GitHub → **Settings → Secrets and variables → Actions**:

| Loại         | Tên                        | Giá trị                                                        |
| ------------ | -------------------------- | ------------------------------------------------------------- |
| **Secret**   | `SUPABASE_ACCESS_TOKEN`    | `sbp_...` — tạo tại https://supabase.com/dashboard/account/tokens |
| **Variable** | `NEXT_PUBLIC_SUPABASE_URL` | `https://<project-ref>.supabase.co`                           |

> Chưa cấu hình 2 giá trị trên thì workflow **bỏ qua** bước SQL (in cảnh báo, không
> làm hỏng deploy) — Vercel vẫn deploy code như thường.

### Làm việc trực tiếp trên nhánh `main`

Để `commit + push` đẩy thẳng lên `main` (không qua nhánh phụ), hãy mở session bằng
**Claude Code CLI** ngay tại thư mục repo:

```bash
git checkout main
claude              # CLI chạy thẳng trên thư mục & nhánh hiện tại
```

> **Lưu ý:** Claude Code **desktop app** luôn tạo một *git worktree* + nhánh `claude/*`
> riêng cho mỗi session (không tắt được) → `push` từ đó chỉ đẩy lên nhánh phụ, **không**
> deploy. Từ worktree, dùng `npm run deploy` để chạy SQL rồi push thẳng `HEAD:main`.

### Thay thế: một lệnh duy nhất từ bất kỳ nhánh nào

```bash
npm run deploy                       # chạy SQL (local) → commit → rebase → push main
npm run deploy -- -m "Nội dung..."   # tự đặt commit message
```

Lệnh này hoạt động từ **mọi nhánh** (kể cả worktree của desktop app): nó chạy SQL bằng
`SUPABASE_ACCESS_TOKEN` trong `.env.local`, commit, rebase theo `origin/main` rồi push
`HEAD:main` để Vercel deploy.
